...
public class GameFrame {
	...
	private static void attackGameForm() {
		...
		JLabel lblPlayer = new JLabel("Current Player - "
		...
		board.setSelectedRegion(diplayCurrentRegionSelectedFrom());
		...
		String[] toRegions = game.getAttackableRegionsList(comboFrom
	...
	private static void attackButton() {
		boolean canMoveFromConquerRegion = game.performAttack(comboFrom
				.getSelectedItem().toString(), comboTo.getSelectedItem()
				.toString());
	...
	protected static void moveToConquerRegion() {
		game.moveTroops(conquerRegionFrom, conquerRegionTo, Integer
				.parseInt(numberReinforcements.getSelectedItem().toString()));
		attackGameForm();
		board.repaint();
	}
	...
	private static void reinforcementGameForm() {
		...
		comboFrom.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (comboFrom.getItemCount() > 0) {
					refreshReinforceComboToValues(comboFrom.getSelectedItem()
							.toString());
					refreshComboTroopsNumber(comboFrom.getSelectedItem()
							.toString());
					board.setSelectedRegion(diplayCurrentRegionSelectedFrom());
				}
			}
		...
		lblBoardTo.setHorizontalAlignment(SwingConstants.LEFT);
		playForm.add(lblBoardTo);
		String[] toRegions = game.getReinforceDestinationRegionsList(comboFrom
				.getSelectedItem().toString());
		comboTo = new JComboBox<String>(toRegions);

		playForm.add(comboTo);

		int maxReinforcements = game.getReinforcements(comboFrom
				.getSelectedItem().toString());
		numberReinforcements = new JComboBox<Integer>();
	...
	protected static void reinforceButton() {
		...
		int troopsNumber = Integer.parseInt(numberReinforcements
				.getSelectedItem().toString());
	...
	protected static void deployButton() {
		...
		int deployNumber = Integer.parseInt(deployCombo.getSelectedItem()
				.toString());
	...
	public static String diplayCurrentRegionSelectedFrom() {
		String currentValue = comboFrom.getSelectedItem().toString();
